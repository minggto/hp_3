...
if __name__ == '__main__':
	...
	# for j in range(1):
		...
		visualpath = visual_basepath +  '/' + dirs[j] + '_post1_takegt' + '/' + num + '/takegt/'
	...
	write_excel(book_name_xls= '/2_data/share/workspace/yym/HP/hp3_visural/test_post1_metrix_count.xls',list_row = [[model_task, num, str(IU[1]), str(mIU), str(fwIU), str(recall),str(precision), str(f1), str(pa), str(mpa)],])
	...
	# f = open(os.path.join(args.statistic_path, 'test_metrix.pckl'), 'wb')
	# pickle.dump([mean_pa,mean_ma,mean_mIU,mean_fwIU], f)
	# f.close()
	pkl_path = '/2_data/share/workspace/yym/HP/hp3_visural/'
	file_name = pkl_path + model_task + '_' + num + '_test_post1_metrix_count.pkl'
