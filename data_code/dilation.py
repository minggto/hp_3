...
class HpInference:
    ...
    def _visualization_statis(self, groudtruth, image_path, visual_path):
        ...
        if not os.path.exists(visual_path+'dilat'):
            os.makedirs(visual_path+'dilat')
        kernel = np.ones((20,20),np.uint8)
        dilation = cv2.dilate(groudtruth,kernel,iterations = 1) #膨胀
        dilat_groudtruth = dilation
        imageio.imwrite(os.path.join(visual_path+'dilat', image_name.split('.png')[0] + '.png'), dilat_groudtruth)
